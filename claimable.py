...
class Coin(pygame.sprite.Sprite):
    def __init__(self, xPos, yPos, blockScaleX, blockScaleY):
        ...
        os_count_frame = len(os.listdir(f"assets/images/tiles/claimable/coin"))
        for i in range(os_count_frame):
            block_img = pygame.image.load(f"assets/images/tiles/claimable/coin/{i}.png")
...
class Koffee(pygame.sprite.Sprite):
    def __init__(self, xPos, yPos, blockScaleX, blockScaleY):
        ...
        os_count_frame = len(os.listdir(f"assets/images/tiles/claimable/koffee"))
        for i in range(os_count_frame):
            block_img = pygame.image.load(f"assets/images/tiles/claimable/koffee/{i}.png")
            block_img = pygame.transform.scale(block_img,  # SCALING happens here
                                               (int(block_img.get_width() * self.blockScaleX),
                                                int(block_img.get_height() * self.blockScaleY)))
            block_img.convert_alpha()
...
class Goffee(pygame.sprite.Sprite):
    def __init__(self, xPos, yPos, blockScaleX, blockScaleY):
        pygame.sprite.Sprite.__init__(self)  # Calling SuperClass to inherit form Sprite
        self.animation_index = 0
        self.animation_list = []
        ...
        os_count_frame = len(os.listdir(f"assets/images/tiles/claimable/Goffee"))
        for i in range(os_count_frame):
            block_img = pygame.image.load(f"assets/images/tiles/claimable/Goffee/{i}.png")
            block_img = pygame.transform.scale(block_img,  # SCALING happens here
                                               (int(block_img.get_width() * self.blockScaleX),
                                                int(block_img.get_height() * self.blockScaleY)))
            block_img.convert_alpha()
